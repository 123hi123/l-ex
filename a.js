...
// @run-at       document-end
// @downloadURL https://update.greasyfork.org/scripts/549533/Discourse%20%E8%A1%A8%E6%83%85%E6%89%A9%E5%B1%95%20%28Emoji%20Extension%20for%20Discourse%29.user.js
// @updateURL https://update.greasyfork.org/scripts/549533/Discourse%20%E8%A1%A8%E6%83%85%E6%89%A9%E5%B1%95%20%28Emoji%20Extension%20for%20Discourse%29.meta.js
// ==/UserScript==
...
(function() {
	...
	function addEmojiToUserscript(emojiData) {
		try {
			console.log("[addEmojiToUserscript] ========== 開始 ==========");
			console.log("[addEmojiToUserscript] 接收到的數據:", emojiData);
			const data = loadDataFromLocalStorage();
			console.log("[addEmojiToUserscript] 載入數據, emojiGroups 數量:", data.emojiGroups.length);
			...
			if (!userGroup) {
				...
				console.log("[addEmojiToUserscript] 創建了新的 user_added 分組");
			}
			console.log("[addEmojiToUserscript] 檢查前 emojis 長度:", userGroup.emojis ? userGroup.emojis.length : "null/undefined");
			// 確保 emojis 是數組
			if (!userGroup.emojis || !Array.isArray(userGroup.emojis)) {
				userGroup.emojis = [];
			}
...
`);
  ...
  `
	...
	function insertEmojiIntoEditor(emoji) {
		...
		// 如果在聊天页面，使用聊天专用插入方法
		if (isChatPage) {
			const chatTextarea = document.querySelector('#channel-composer, textarea.chat-composer__input, textarea[data-chat-composer-context="channel"]');
			if (!chatTextarea) {
				console.error("[Emoji Extension] ❌ 找不到聊天输入框");
				return;
			}
			...
			const cursorPos = chatTextarea.selectionStart || 0;
			const newValue = currentValue.substring(0, cursorPos) + insertText + currentValue.substring(cursorPos);
			
			// 设置新值
			nativeSetter.call(chatTextarea, newValue);
			
			// 设置光标位置
			const newCursorPos = cursorPos + insertText.length;
			chatTextarea.selectionStart = newCursorPos;
			chatTextarea.selectionEnd = newCursorPos;
			
			// 触发事件
			chatTextarea.dispatchEvent(new InputEvent('input', { bubbles: true, inputType: 'insertText' }));
			chatTextarea.dispatchEvent(new Event('change', { bubbles: true }));
			
			// 聚焦
			chatTextarea.focus();
			
			console.log("[Emoji Extension] ✅ 表情包已插入到聊天输入框");
			return;
		}
	...
	function createMobileEmojiPicker(groups) {
		...
		groups.forEach((group, index) => {
			...
			(group.id === "user_added" ? [...group.emojis].reverse() : group.emojis).forEach((emoji) => {
	...
	// 设置 textarea 点击事件处理器 - 点击时刷新扩展按钮
	function setupTextareaClickHandlers() {
		const textareas = document.querySelectorAll('textarea.chat-composer__input, textarea[data-chat-composer-context="channel"]');
		textareas.forEach((textarea) => {
			...
			textarea.addEventListener('click', () => {
				console.log('[Emoji Extension] Textarea clicked, refreshing extension buttons...');
				// 使用 attemptInjection 而不是手动操作，它会自动检查是否已存在
				setTimeout(() => {
					attemptInjection();
				}, 50);
			});
			console.log('[Emoji Extension] Click handler attached to textarea');
		});
	}
	...
	function setupPasteHandlers() {
		// 同时监听 textarea 和外层容器
		const textareas = document.querySelectorAll('textarea.chat-composer__input, textarea[data-chat-composer-context="channel"], textarea.d-editor-input');
		...
		textareas.forEach((textarea) => {
			...
			console.log('[Emoji Extension] Paste handler attached to textarea');
		});
		
		// 为外层容器添加粘贴监听（备用）
		containers.forEach((container) => {
			if (container.dataset.emojiExtensionPasteHandler) return;
			container.dataset.emojiExtensionPasteHandler = 'true';
			
			container.addEventListener('paste', (e) => {
				const textarea = container.querySelector('textarea.chat-composer__input, textarea[data-chat-composer-context="channel"]');
				if (textarea) {
					handlePaste(e, textarea);
				}
			});
			console.log('[Emoji Extension] Paste handler attached to container');
		});
	}
	...
	async function handlePaste(e, textarea) {
		...
		if (!items) {
			console.log('[Emoji Extension] No clipboard items found');
			return;
		}
		
		console.log('[Emoji Extension] Paste event detected');
		console.log('[Emoji Extension] Items count:', items.length);
		console.log('[Emoji Extension] Textarea:', textarea.className);
		console.log('[Emoji Extension] Current page:', window.location.pathname);
		
		// 打印所有 item 类型
		for (let i = 0; i < items.length; i++) {
			console.log(`[Emoji Extension] Item ${i}: type="${items[i].type}", kind="${items[i].kind}"`);
		}
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (item.type === 'text/plain') {
				// 使用 Promise 来处理异步文本获取
				try {
					const text = await new Promise((resolve) => {
						item.getAsString((str) => resolve(str));
					});
					
					console.log('[Emoji Extension] Text content:', text);
					
					// 检查是否已经是 Markdown 格式（避免递归包装）
					if (text && text.trim().startsWith('![') && text.includes('](')) {
						console.log('[Emoji Extension] ⚠️ Already Markdown format, skipping');
						return; // 让浏览器默认处理
					}
					
					// 检查是否是图片链接
					if (text && (text.match(/\.(jpg|jpeg|png|gif|webp|bmp)(\?.*)?$/i) || 
					    text.includes('/uploads/') || 
					    text.match(/https?:\/\/.*\.(jpg|jpeg|png|gif|webp|bmp)/i))) {
						e.preventDefault();
						console.log('[Emoji Extension] ✅ Image URL detected, inserting:', text.trim());
						insertImageLink(textarea, text.trim());
						return;
					} else {
						console.log('[Emoji Extension] ❌ Not an image URL');
					}
				} catch (err) {
					console.error('[Emoji Extension] Error reading text:', err);
				}
			}
		}
	...
	function insertImageLink(textarea, imageUrl) {
		...
		console.log('[Emoji Extension] Inserting image, isChatPage:', isChatPage, 'URL:', fullUrl);
		
		if (isChatPage) {
			// 聊天页面：使用 native setter 直接插入
			const imageMarkdown = `![image](${fullUrl})`;
			...
			const newValue = currentValue.substring(0, cursorPos) + imageMarkdown + currentValue.substring(cursorPos);
			...
			console.log('[Emoji Extension] ✅ 图片已插入到聊天输入框');
		} else {
			// 普通页面：使用 Markdown 格式插入
			insertImageAsMarkdown(textarea, fullUrl);
		}
	}
	
	// 为聊天页面插入图片
	function insertImageToChatComposer(textarea, imageUrl) {
		console.log('[Emoji Extension] Inserting image to chat composer');
		...
		events.forEach(event => {
			try {
				...
				console.warn('[Emoji Extension] Failed to dispatch event:', e);
			}
		});
		...
		// 方法 2：尝试通过 Ember/Glimmer 的方式更新
		try {
			// 查找 Ember 组件实例
			const emberView = textarea.closest('[id^="ember"]');
			if (emberView) {
				// 触发 Ember 的事件
				const customEvent = new CustomEvent('ember-action', {
					bubbles: true,
					detail: { value: newValue }
				});
				emberView.dispatchEvent(customEvent);
			}
		} catch (e) {
			console.warn('[Emoji Extension] Ember update failed:', e);
		}
		
		// 方法 3：使用 MutationObserver 通知
		setTimeout(() => {
			// 再次触发 input 事件
			textarea.dispatchEvent(new Event('input', { bubbles: true }));
		...
		console.log('[Emoji Extension] Chat composer image inserted');
	}
	
	// 为普通编辑器插入 Markdown 图片
	function insertImageAsMarkdown(textarea, imageUrl) {
		...
		// 构建 Markdown 图片语法
		const imageMarkdown = `${prefix}![image](${imageUrl})${suffix}`;
		...
		console.log('[Emoji Extension] Markdown image link inserted successfully');
	}
	...
	function attemptInjection() {
		...
		// 为所有 textarea.chat-composer__input 添加点击事件监听器
		setupTextareaClickHandlers();
		// 为所有 textarea 添加粘贴事件监听器
		setupPasteHandlers();
		return {
			injectedCount,
	...
	function setupNetworkListeners() {
		...
		if (typeof window.fetch !== 'undefined') {
			...
			window.fetch = function(...args) {
				...
				if (typeof url === 'string') {
					// 检测聊天室切换：/chat/api/channels/*/messages（更快更通用）
					if (url.includes('/chat/api/channels/') && url.includes('/messages')) {
						console.log('[Emoji Extension] 🔄 检测到聊天室切换 (messages):', url);
						// 延迟100ms后尝试注入，确保聊天输入框已加载
						setTimeout(() => {
							console.log('[Emoji Extension] 尝试在聊天室注入按钮...');
							attemptInjection();
						}, 100);
					}
					// 也检测 drafts（作为备用）
					else if (url.includes('/chat/api/channels/') && url.includes('/drafts')) {
						console.log('[Emoji Extension] 🔄 检测到聊天室切换 (drafts):', url);
						setTimeout(() => {
							console.log('[Emoji Extension] 尝试在聊天室注入按钮...');
							attemptInjection();
						}, 100);
					...
					// 检测普通编辑器相关请求
					else if (url.includes('/drafts/') || url.includes('/posts/') || url.includes('/t/')) {
		...
		if (typeof window.XMLHttpRequest !== 'undefined') {
			...
			XMLHttpRequest.prototype.open = function(method, url, ...rest) {
				if (typeof url === 'string') {
					// 检测聊天室切换：messages 请求（更快）
					if (url.includes('/chat/api/channels/') && url.includes('/messages')) {
						console.log('[Emoji Extension] 🔄 XHR 检测到聊天室切换 (messages):', url);
					...
					// 也检测 drafts（备用）
						...
						console.log('[Emoji Extension] 🔄 XHR 检测到聊天室切换 (drafts):', url);
						this.addEventListener('load', function() {
							setTimeout(() => {
								console.log('[Emoji Extension] 尝试在聊天室注入按钮...');
								attemptInjection();
